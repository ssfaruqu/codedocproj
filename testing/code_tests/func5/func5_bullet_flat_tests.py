...
def bullet_flat_test_cases(): # pragma: no cover
    try:
        ...
        count += 1 # Test case 2: Player has 3 consecutive pieces vertically
        assert func5('X', 0, 0, [['X', 'O', ' '], ['X', 'O', ' '], ['X', ' ', 'O']]) == 10000

        count += 1 # Test case 3: Player has 3 consecutive pieces horizontally
        assert func5('O', 1, 0, [['X', 'X', 'O'], [' ', 'O', 'O'], ['X', ' ', 'O']]) == -10000

        count += 1 # Test case 4: Player has 3 consecutive pieces diagonally (top-left to bottom-right)
        assert func5('X', 0, 0, [['X', 'O', ' '], [' ', 'X', ' '], ['O', 'O', 'X']]) == 10000

        count += 1 # Test case 5: Player has 3 consecutive pieces diagonally (top-right to bottom-left)
        assert func5('X', 0, 2, [[' ', 'O', 'X'], [' ', 'X', ' '], ['X', 'O', 'O']]) == 10000

        count += 1 # Test case 6: Player has 2 consecutive pieces in all directions
        assert func5('X', 1, 1, [['X', 'X', 'O'], ['O', 'X', ' '], ['O', ' ', 'O']]) == 4

        count += 1 # Test case 7: Player has 1 piece surrounded by opponents
        assert func5('X', 1, 1, [['O', 'X', 'O'], ['X', 'O', 'X'], ['O', 'X', 'O']]) == 1

        count += 1 # Test case 8: Player has 3 consecutive pieces with some opponents in between
        assert func5('X', 0, 0, [['X', 'O', 'X'], ['O', 'X', ' '], ['X', 'O', 'X']]) == 10000

        count += 1 # Test case 9: Player has no consecutive pieces, all opponents
        assert func5('X', 1, 1, [['O', 'X', 'O'], ['X', 'O', 'X'], ['O', 'X', 'O']]) == -1

        count += 1 # Test case 10: Player has 3 consecutive pieces diagonally, with a gap
