...
sheet = client.open("CodeDocProj").get_worksheet(3)
categories = ["func name", "prompt style",  "score", "orig tok", "gen tok", "tok_ratio", "leq_bin", "coverage ratio", "fail ratio"]
...
def print_score(name, leq, cover, fail, prompt_style):
    print(f"leq_bin= {leq}, cover_ratio= {cover}, fail_ratio= {fail}, gen_tok= {gi.getFuncSummLen()[name][prompt_style]},  orig_tok= {oi.getFuncLen()[name]}")
    score = score_eq(name, leq, cover, fail, prompt_style)
    print(score)

    # add data to spreadsheet
    row = [name, gi.getTestCaseNames()[prompt_style], score, oi.getFuncLen()[name], gi.getFuncSummLen()[name][prompt_style], (gi.getFuncSummLen()[name][prompt_style]/oi.getFuncLen()[name]), leq, cover, fail]
